 ...
 *      -Boolean:col -> booleano que indicará si el puzzle es de colores, si no se ignorará.
 * Salidas:
 * */
...
this.isSolved = function ()
{
    ...
    for(var i=1; i<7; i++)
    {
        ...
        for(var j=0; j<faceCubes.length-1; j++)
        {
            ...
            if(currentDif < 0)
                currentDif+=360;
            //Si es un puzzle de colores y vamos a comparar con la seccion central de la cara, la saltamos
            //ya que con un puzzle de colores no importa el giro que tenga la seccion central
            if(col && numberOfCubes == 3 && j==3)
            {
                j++;
                nextFaceIndex = getFaceIndex(faceCubes[j+1],i);
                //Comparamos cada imagen de la cara con la imagen del siguiente cubo
                if(faceCubes[j].getFace(currentFaceIndex.x).getImg() != faceCubes[j+1].getFace(nextFaceIndex.x).getImg())
                    return false;
