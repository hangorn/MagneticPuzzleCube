...
/*
 * Nombre: Constructor
 * Sinopsis: Constructor de la clase MovingPuzzleController.
 * Entradas:
    ...
    {        
        ...
        if(movDown && rotDown)
        {
            ...
            if(!event.ctrlKey)
            {
                rotationStore.x = rotationStore.y = 0;
                //Giramos la figura segun la sensibilidad
                pv.rotateShape(SELECTED, 0,0, - sensitivity * (mouseY - lastMouseY) - sensitivity * (mouseX - lastMouseX));
            }
            //Si esta pulsada la tecla CTRL
            else
            {
...
/*
 * Nombre: enableAnimation
 * Sinopsis: Método que iniciará la animación de los cubos.
 * Entradas:
 * Salidas:
 * */
...
/*
 * Nombre: animateCubes
 * Sinopsis: Método para mover los cubos.
 * Entradas:
 * Salidas:
 * */
...
{    
    if(animationEnabled)
    {
        ...
        for(var i=0; i< objects.length; i++)
        {
            ...
            {
                //Movemos cada cubo
                objects[i].position.addSelf(new THREE.Vector3(objects[i].dir.x*(Math.random()*20+40), objects[i].dir.y*(Math.random()*20+40), 0));
